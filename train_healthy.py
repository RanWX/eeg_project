...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        train_data_list, val_data_list, test_data_list = data_process.split_healthy_data_by_proportion(src_path="",
                                                                                                         seed=0,
                                                                                                         **proportion)
        print(len(train_data_list),len(test_data_list),len(val_data_list))
        save_str = train.train_model(train_data_list, val_data_list, test_data_list, "leave_one_session_out")
        result_list.append(save_str)
        print(save_str)
    if not Path.exists(Path("result_healthy_10_fold")):
        Path("result_healthy_10_fold").mkdir()
    if type == "*":
        type = "1122"
    with open("result_healthy_10_fold/result_healthy_10_fold_{}.txt".format(type), "w") as f:
        f.write("\n".join(result_list))
