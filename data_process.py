...
def split_dataset_by_proportion(event, src_path="", proportion=0.8, seed=0):
    '''
    根据需要将数据按比例分割
    ...
    try:
        ...
        healthy_train_len = int(len(data_healthy_list) * proportion)
        unhealthy_train_len = int(len(data_unhealthy_list) * proportion)
        ...
        val_data_list = [data_healthy_list[i] for i in healthy_index_perm_list[healthy_train_len:]] + [
            data_unhealthy_list[j] for j in unhealthy_index_perm_list[unhealthy_train_len:]]
...
if __name__ == '__main__':
    train_data_list, val_data_list = split_dataset_by_proportion("*")
