...
class train():
    ...
    def train_model(self, time_point, train_data_list, val_data_list, test_data_list, i, cv_type):
        # TODO: no cuda
        ...
        for epoch in range(num_epochs):
            ...
            # save_list.append(
            #     'Epoch [{}/{}], Loss: {:.4f}, Acc: {:.4f}'.format(epoch + 1, num_epochs, losses[-1], accs[-1]))
            ...
            with torch.no_grad():
                ...
                # save_list.append('Evaluation Loss:{:.4f}, Acc: {:.4f}'
                #                  .format(Loss_val[-1], Acc_val[-1]))
...
if __name__ == '__main__':
    ...
    train_con = train()
    # set parameters
    train_con.set_parameter(cv='Leave_one_session_out',
                            model='TSception',
                            number_class=2,
                            sampling_rate=1000,
                            random_seed=42,
    ...
    for i in range(k):
        train_data_list, val_data_list, test_data_list = data_process.split_dataset_by_condition("", event_condition,
                                                                                                 attack_condition,
                                                                                                 src_path="", seed=0,
                                                                                                 **proportion)
        print("************ {} fold **********".format(str(i)))

        save_str = train_con.train_model(None, train_data_list,
                                         val_data_list,
                                         test_data_list, i,
    ...
    event_condition = event_condition if event_condition != "*" else "1122"
    attack_condition = attack_condition if attack_condition != "*" else "12"
    with open("result_conditon_10_fold/result_condition_10_fold_{}_{}.txt".format(attack_condition, event_condition),
              "w") as f:
        f.write("\n".join(result_list))
